...
NEW_ANSWER_POOL = [
    ("What is natural language processing (NLP) and what are its main applications?", ["Natural language processing, or NLP, is a branch of artificial intelligence that lets computers understand, interpret, and generate human language. Main applications are machine translation, sentiment analysis, chatbots, speech recognition, and text summarization. It brings together linguistics, computer science, and machine learning to work with unstructured text.", "NLP is a field of AI that helps computers understand human language. Applications include translation, sentiment analysis, chatbots, and speech recognition. It combines linguistics and machine learning.", "Natural language processing lets machines read and respond to text and speech. Translation, text summarization, and spam filtering all depend on it. The field sits at the intersection of computer science and linguistics.", "NLP is about computers understanding human language, like in translation and chatbots.", "NLP teaches computers to work with language, for example in translation tools and voice assistants."]),
    ("Explain the difference between stemming and lemmatization.", ["Stemming and lemmatization both normalize words to their base form. Stemming strips suffixes with simple rules, so it is fast but can create non-words like 'fli' from 'flies'. Lemmatization uses vocabulary and morphological analysis to return the real dictionary form (the lemma), such as 'fly' from 'flies'. Lemmatization is more accurate but slower and requires a lexicon; stemming is quicker but less precise.", "Stemming removes suffixes from words to get a base form, while lemmatization returns the true dictionary form using vocabulary and morphology. Lemmatization is more accurate but slower.", "Stemming chops word endings using simple rules, which is fast but can create non-words. Lemmatization looks up the real root word in a dictionary, so it is more accurate even though it takes longer.", "Stemming and lemmatization both reduce words to base forms. Lemmatization is more accurate than stemming.", "Stemming shortens words quickly, lemmatization finds the proper root, and lemmatization is the more careful method."]),
    ("What is a bag-of-words model and what are its limitations?", ["The bag-of-words model represents text as a collection of word counts without considering grammar or word order, so each document becomes a frequency vector. Its main limitations are that it loses word order and meaning, produces high-dimensional and sparse vectors, and cannot recognize synonyms, because similar words map to different dimensions. TF-IDF weighting and word embeddings help overcome some of these issues.", "Bag-of-words counts word frequencies in a document and ignores grammar and word order. It loses semantics and creates large sparse vectors, and it cannot handle synonyms well.", "A bag-of-words model turns text into simple word counts without any grammar. Because order and meaning are dropped, the vectors become huge and sparse, and words with the same meaning are treated as unrelated.", "Bag-of-words counts how often words appear, ignoring order, and it cannot capture meaning or synonyms.", "Bag-of-words just counts words and ignores how they are arranged, so similar words end up looking different."]),
    ("Describe how a transformer-based language model works at a high level.", ["A transformer uses self-attention to process text. Tokens are converted to embeddings, and attention layers compute how every token relates to all others in parallel, capturing context. Multi-head attention, positional encodings, and feed-forward layers make up the encoder-decoder blocks. The model is pretrained on huge text corpora with objectives like masked language modeling, then fine-tuned for specific tasks.", "Transformers use self-attention to relate every token to every other token, capturing context in parallel. They are pretrained on large text and fine-tuned for tasks like translation and classification.", "A transformer works by giving each word attention over all the other words in the sentence, which lets it understand context. The model is first trained on huge amounts of text and then adjusted for specific jobs.", "A transformer is a model that uses attention to understand context in text and is trained on large amounts of data.", "Transformers are models that use attention to understand context and are trained on big text collections."]),
    ("What is the difference between supervised and unsupervised learning in NLP?", ["Supervised learning in NLP trains models on labeled data, such as sentiment or part-of-speech labels, to predict outputs for new examples. Unsupervised learning finds structure in unlabeled text, for example topic modeling with LDA or word embeddings such as Word2Vec. Supervised methods are task-specific and require labeled datasets, while unsupervised methods discover patterns without labels.", "Supervised NLP uses labeled data to train models for tasks like sentiment analysis, while unsupervised NLP finds patterns in unlabeled text, like topics or word embeddings.", "In supervised learning the model learns from examples with known answers, such as reviews already marked positive or negative. Unsupervised learning gets raw text with no labels and discovers structure by itself, like grouping similar documents.", "Supervised learning uses labeled data; unsupervised learning works without labels.", "Supervised learning trains with labeled examples, unsupervised learning works with unlabeled text."]),
    ("What is data mining and what are its main tasks?", ["Data mining is the process of discovering patterns, correlations, and useful knowledge from large datasets using statistics, machine learning, and database techniques. The main tasks are classification, clustering, regression, association rule mining, anomaly detection, and prediction. It is widely used for customer segmentation, fraud detection, and market basket analysis.", "Data mining finds patterns in large datasets using statistics and machine learning. Main tasks include classification, clustering, and association rule mining, used in applications like fraud detection.", "Data mining extracts useful knowledge from big data collections. It covers jobs such as predicting outcomes, grouping similar records, and finding items that are often bought together, which helps businesses detect fraud and understand customers.", "Data mining is finding patterns in data, like grouping customers or detecting fraud.", "Data mining looks for useful patterns in large data, for example customer groups or fraud signals."]),
    ("Explain the difference between classification and clustering.", ["Classification is a supervised task where each data item is assigned to predefined classes using a labeled training set, such as spam detection. Clustering is an unsupervised task that groups similar items together without labels, such as customer segmentation. In short, classification predicts known labels, while clustering discovers unknown groups.", "Classification assigns data to known classes using labeled examples, while clustering groups similar data without labels. Classification is supervised; clustering is unsupervised.", "Classification is used when the categories are already known, and the model learns from past labeled cases to sort new ones. Clustering, in contrast, discovers groups on its own from the data, with no pre-defined categories.", "Classification puts data into known groups; clustering finds groups by itself.", "Classification sorts items into predefined groups, clustering creates new groups from the data."]),
    ("What is association rule mining and how are support and confidence computed?", ["Association rule mining finds relationships between items in transaction data, like market basket analysis. For a rule X implies Y, support is the fraction of transactions containing both X and Y, while confidence is the fraction of transactions with X that also contain Y. Lift compares how much more likely Y is when X is present.", "Association rule mining finds item relationships in transactions, like bread and butter. Support is the frequency of both items together, and confidence is how often Y appears when X appears.", "Association mining looks at purchase data to find items that go together. A rule's support tells us the share of transactions containing both products, and its confidence tells us how reliably the second product appears with the first.", "Association rules find items often bought together. Support and confidence measure how strong the rule is.", "Association rules show which items are bought together, measured by support and confidence."]),
    ("Describe the k-means clustering algorithm and its limitations.", ["K-means partitions data into k clusters by minimizing within-cluster variance. It initializes k centroids, assigns each point to the nearest centroid, recalculates centroids as the mean of their points, and repeats until convergence. Limitations include having to choose k in advance, sensitivity to initialization and outliers, assuming spherical clusters, and the risk of local optima.", "K-means groups data into k clusters using centroids and repeated assignments. Its limits are needing k in advance and being sensitive to outliers and initialization.", "The k-means algorithm divides data into k clusters by repeatedly moving cluster centers to the average of their members. The user must choose k beforehand, and unusual data points can pull the centers off, so results are not always stable.", "K-means clusters data into k groups. You must choose k and outliers can affect the result.", "K-means puts data into k clusters, but the number k must be set first and outliers can cause problems."]),
    ("What is the difference between training data and test data, and why is it important?", ["Training data is used to fit a model's parameters, and test data measures performance on unseen examples to check generalization. Separating the two prevents overfitting, where the model memorizes training data but fails on new data. Holdout sets and cross-validation provide more reliable performance estimates.", "Training data fits the model and test data checks it on new examples. This separation prevents overfitting and shows how well the model generalizes.", "The model learns its parameters from the training set, while the test set measures how it performs on data it has never seen. Keeping these sets separate is what reveals whether the model truly generalizes or has just memorized the examples.", "Training data trains the model and test data checks if it works on new data.", "Training data is for learning, test data is for checking performance on new examples."]),
    ("What is an ERP system and what are its main benefits?", ["An ERP system is integrated software that manages core business processes such as finance, human resources, procurement, manufacturing, sales, and inventory in one unified database. The main benefits are real-time data visibility, process automation, less data duplication, better decision making, and smoother workflows across departments.", "ERP is software that integrates business processes like finance, HR, and inventory into one system. Benefits include real-time data, automation, and better decisions.", "An ERP system connects all departments through a single shared database, so sales, stock, and accounts stay in sync. This gives managers up-to-date information, reduces duplicate entries, and speeds up everyday operations.", "ERP is a system that connects company departments together using one database.", "ERP links departments like finance and inventory through one shared system."]),
    ("Explain the difference between on-premise and cloud ERP.", ["On-premise ERP runs on the company's own servers, giving full control and data ownership but requiring high upfront cost and in-house IT maintenance. Cloud ERP is hosted by the vendor as SaaS and accessed over the internet, offering lower upfront cost, automatic updates, and scalability, but relying on connectivity and vendor security.", "On-premise ERP is installed on company servers with high upfront cost, while cloud ERP is hosted by the vendor with lower cost and easier scaling.", "With on-premise ERP the company buys and maintains its own hardware, which means full control but high initial investment. Cloud ERP is rented as a service over the internet, so the firm pays less at the start and can grow quickly, though it depends on the vendor's infrastructure.", "On-premise ERP runs on your own servers; cloud ERP runs on the vendor's servers.", "On-premise means installing on your own machines, cloud means using the vendor's online service."]),
    ("What is master data in an ERP system and why is it important?", ["Master data is the core reference data shared across the organization, such as customers, vendors, products, employees, and the chart of accounts. It matters because it creates a single source of truth, ensuring all modules use consistent information. Master data management (MDM) keeps this data accurate, consistent, and properly governed.", "Master data is shared reference data like customers and products. It gives the organization a single source of truth so all modules use the same information.", "Master data covers the core records that every department needs, such as suppliers, products, and employees. When this data is managed well, all parts of the ERP system work from the same accurate figures instead of each keeping its own copies.", "Master data is the important shared data like customer and product lists used across the system.", "Master data is the key shared information, such as customers and products, that the whole system uses."]),
    ("Describe the main modules typically found in an ERP system.", ["Typical ERP modules are finance and accounting, human resources, procurement, inventory and supply chain, manufacturing, sales and distribution, and customer relationship management (CRM). All modules share a single database, so a transaction like a sales order automatically updates inventory and finance records.", "ERP modules include finance, HR, procurement, inventory, sales, and CRM. They share one database so data flows between departments automatically.", "A typical ERP package comes with separate modules for accounting, staff management, purchasing, warehousing, production, and customer relations. Because they all read from the same database, an action in one module updates the others right away.", "ERP has modules for finance, HR, inventory, and sales that work together.", "ERP consists of modules like finance, HR, and sales that share data with each other."]),
    ("What are the key steps in an ERP implementation project?", ["The key steps are requirements analysis, vendor and package selection, planning, configuration, data migration, integration, testing, training, go-live, and post-implementation support. Change management and executive sponsorship are critical for success.", "An ERP project goes through requirements, configuration, data migration, testing, training, and go-live. Good change management is important.", "Implementing ERP starts with understanding the requirements and choosing the right system, then configuring it, moving the old data over, testing everything, training users, and finally launching. Managing staff expectations along the way is a major factor in success.", "ERP implementation includes planning, setup, testing, training, and going live.", "ERP projects need planning, configuration, testing, and training before going live."]),
]

NEW_STUDENT_LEVELS = {
    # student email -> answer index in NEW_ANSWER_POOL (each student gets a UNIQUE text)
